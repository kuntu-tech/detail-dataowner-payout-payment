...
const ConnectExisting = ({ onBack, onConnect }: ConnectExistingProps) => {
  ...
  const [errors, setErrors] = useState<{ publishable?: string; secret?: string; accessId?: string }>({});
  ...
  const handleAdvancedConnect = () => {
    const newErrors: { publishable?: string; secret?: string } = {};
    if (!publishableKey) newErrors.publishable = "Publishable key is required";
    if (!secretKey) newErrors.secret = "Secret key is required";
      ...
      {activeTab === "recommended" && (
        <div className="max-w-2xl rounded-xl border border-border bg-card p-8">
          ...
          <div className="space-y-3">
            ...
            />
            {errors.accessId && <p className="text-sm text-destructive">{errors.accessId}</p>}
